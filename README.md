...
GHI-S2S consists of approximately 150 tasks that follow a predeifined schedule. The tasks are grouped in to 50 job files in order to optimize computer resources.  
...
create run-directories, links, generate bash script files, and set up the S2S forecast experiment each month.

The ghis2s Python package can be called either from within an external Python program or run from the command line.
