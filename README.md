...
We present **ghis2s** as a Python package that can be called either from within an external Python program or run from the command line. The **ghis2s** package provides a Python script, [*run_s2s_fcast.py*](https://github.com/smahanam/LISF-1/blob/parallelizing/lis/utils/usaf/S2S/ghis2s/cylc_script/run_s2s_fcast.py) , for importing **ghis2s**, and installing **Cylc** workflow. Optionally, **run_s2s_fcast.py** is also good to execute monthly forecasts in the SLURM system. Users are welcome to copy **run_s2s_fcast.py** into their working directories and modify it as needed.  
